 ...
 * This ensures the language setting is synchronised with the server-side user
 * attribute so that the correct locale is applied on the next restart.
 */
   ...
   public void afterLogin(NXCSession session, Display display)
   {
      try
      {
         ...
         if ((serverLanguage != null) && !serverLanguage.isEmpty())
         {
            ...
            PreferenceStore.getInstance().set(LanguagePage.LANGUAGE_PREFERENCE_KEY, serverLanguage);
            Locale.setDefault(LocalizationHelper.localeFromLanguageCode(serverLanguage));
         }
      }
      catch(Exception e)
      {
