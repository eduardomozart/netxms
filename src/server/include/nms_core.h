...
public:
   ...
   virtual void close(bool success, bool deleteOnFailure = true, bool replaceLocked = false) override;
...
bool NXCORE_EXPORTABLE SanitizeSVGFile(const TCHAR *fileName);
...
void NXCORE_EXPORTABLE SendNotification(const TCHAR *name, TCHAR *recipient, const TCHAR *subject, const TCHAR *message, uint32_t eventCode, uint64_t eventId, const uuid& ruleId, const TCHAR *ruleDescription = nullptr);
void NXCORE_EXPORTABLE SendNotification(const TCHAR *name, TCHAR *recipient, const TCHAR *subject, const TCHAR *message, const Event *event, const shared_ptr<NetObj>& sourceObject, const uuid& ruleId, const TCHAR *ruleDescription = nullptr);
