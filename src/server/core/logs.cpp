...
static NXCORE_LOG s_logs[] =
{
   ...
   { L"ConnectionHistory", L"connection_history", L"record_id", L"switch_id", SYSTEM_ACCESS_SEARCH_NETWORK,
      "Connection history log. Tracks MAC address connections, disconnections, and movements on switch ports. "
      "Contains MAC address, resolved IP address, node ID, switch and interface where the event occurred, "
      "VLAN ID, and event type (connect, disconnect, move).",
      {
         { L"record_id", L"Record ID", LC_INTEGER, LCF_RECORD_ID },
